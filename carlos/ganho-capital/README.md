...
OperacaoDeCompra
OperacaoDeVenda
OperacaoDeVendaAcimaDoLimiteDe20000
...
```SHELL
docker run -it --rm ganho-capital sh -c "java -jar ./app/app.jar"
```
...
```SHELL
docker run -i --rm ganho-capital sh -c 'java -jar ./app/app.jar' < input.txt
```
Ou
```SHELL
